...
export default function EventDetail({ event, participants, groups, isAdmin, onEdit, onDelete, onClose }) {
  ...
  // All unique members (individual + via groups)
  const groupMemberIds = assignedGroups.flatMap(g => g.memberIds || []);
  const allMemberIds = [...new Set([...(event.participants || []), ...groupMemberIds])];
  const allMembers = allMemberIds.map(id => participants.find(p => p.id === id)).filter(Boolean);
        ...
        <div style={{ marginBottom:16 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
            {allMembers.map(p => {
              ...
              const groupNames = assignedGroups.filter(g => (g.memberIds||[]).includes(p.id)).map(g => g.name);
              return (
